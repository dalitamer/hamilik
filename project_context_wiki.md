...
## Header Separator, Contact Links, Mobile Scrollspy, Campaign Visuals (2026-07-07)
- Removed the `×` separators (`<span class="x">`) between header logos on all five pages; purged `.brand .x` rules from `assets/styles.css`. Footer/hero text `×` usages kept.
- Added `Hamilik Okulu Vakfı → hamilikokulu.org` as the first entry of the contact page `Kurumsal bağlantılar` list; intro copy now says `kurumların resmi sayfaları`.
- (Committed and pushed as `06152a1` + `b7235d9`; Render deployed.)
- Fixed the mobile module-nav scrollspy on `program-detayi.html`: replaced the `threshold: 0.4` IntersectionObserver in `assets/detail.js` with a viewport-independent probe-line scrollspy (~38% of viewport height) plus `setActive`/`centerChip` helpers; the active chip now updates on tall mobile cards and auto-centers horizontally inside `#modnav` without vertical page jumps. Mobile chips got `min-height: 40px`, strip edge padding, and `overflow-y: hidden` in `assets/detail.css` (`max-width: 560px` tier). Hash deep links (`#m0`–`#m7`) still highlight and center the chip.
- Client sent official campaign visuals ("Hamilik Okulu Kırşehir'de — Kendini Keşfet, Farkını Geliştir, Geleceğe Değer Kat!"): extracted from `C:\Users\Durmu\Downloads\hamilik_okulu_kırsehir_site.pdf` into `assets/visual-kampus-hero-light.jpg`, `assets/visual-kampus-hero-light-2.jpg`, and `assets/visual-kampus-hero-dark.jpg` (dark variant currently unused, kept for future dark surfaces).
- Placed `visual-kampus-hero-light.jpg` as a wide `.pp-visual` figure inside the `partnership-panels` section on `program-detayi.html` (styles in `assets/detail.css`), and `visual-kampus-hero-light-2.jpg` as a `.partner-visual` figure in the homepage partnership band (styles in `assets/home.css`). Both lazy-loaded with descriptive Turkish alt text and natural aspect ratio.
- Client-approved program definition text ("Hamilik Okulu Kırşehir, öğrencilerin meslekle kurduğu ilişkiyi derinleştiren; ... bütüncül bir bakış kazandırmayı amaçlar.") replaced the `program-detayi.html` `.dhero` lead paragraph.
- The two portrait "Hamilik Okulunda seni neler bekliyor" posters the client pasted in chat were not available as local files; user asked to save them so they can be integrated (candidate spot: homepage `Yolculuğun Sekiz Durağı` section).
- Verification: partnership/homepage visuals return HTTP 200 and render with no horizontal overflow; probe-line scrollspy algorithm verified geometrically at 375px (picks m0–m7 correctly, falls back to `Tümü` above the first card). Note: the embedded preview browser reports `document.visibilityState === "hidden"`, so rAF/scroll events do not fire there — scroll-driven behavior must be validated in a real browser after deploy.
